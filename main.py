...
# Global variables
watermark = "NIKHIL"  # Default value
count = 0
userbot = None
...
BUTTONSCONTACT = InlineKeyboardMarkup([[InlineKeyboardButton(text="📞 Contact", url="https://t.me/STUDENTZZZBOT")]])
keyboard = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(text="🛠️ Help", url="https://t.me/STUDENTZZZBOT")        ],
    ]
)
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                url = f"https://marco-cp-api.vercel.app/marco?url={url}@ITSMARCOOFFICIAL&user_id={7445620075}"
                mpd, keys = helper.get_mps_and_keys(url)
                url = mpd
                keys_string = " ".join([f"--key {key}" for key in keys])

            elif "classplusapp" in url:
                signed_api = f"https://marco-cp-api.vercel.app/marco?url={url}@ITSMARCOOFFICIAL&user_id={7445620075}"
